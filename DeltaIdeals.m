...
{Given a fractional WR-ideal I in the DieudonneAlgebra of isog, returns Delta^-1(I), which is a Z[pi,q/pi]-ideal in the DeligneAlgebra.}
        if not assigned I`Delta_inverse_ideal then
            R:=ZFVOrder(isog);
            _,_,_,_,_,_,_,Delta_map,WR,_,OA_as_abelian_group_data:=DieudonneAlgebraCommEndAlg(isog);
...
{Given an isogeny class isog, a fractional WR-ideal J and a sequence of fractional WR-ideals Is of the DieudonneAlgebra it returns a sequence IIs and an integer m0 such that, for each i: Is[i] is Delta-isomorphic to IIs[i], each IIs[i] is inside J, and m0=Max(Valuation(p,Index(J,IIs[i])) is small.}
    ...
    // but only a quotient isomorphic to its p-part.
        ...
        ZBasisLLL(B);
        ind:=Index(A,B);
        v_ind:=Valuation(ind,p);
        if v_ind eq 0 then
            return 0; // A_p = B_p
        end if;
        prec:=Max(prec,v_ind+1);
        Zp:=pAdicRing(p,prec);
        zbB:=ZBasis(B);
        M:=Matrix(Zp,AbsoluteCoordinates(zbB,A));
        n:=#zbB;
