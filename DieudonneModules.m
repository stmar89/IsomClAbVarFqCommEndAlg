...
declare attributes AlgEtQIdl : DeltaEndomorphismRing,
                               SlopeE;
// local_basis_and_coeffs:=function(S,R,P,m)
// // Let m:R->S be a ring homomorphism between order is possibly different algebras, and P a maximal ideal of R.
// // Assume that S_P is a free R_P-algebra.
// // Returns a sequence of n elements of S representing a basis of S_P over R_P, together with a function S->L^n, 
// // where L is the algebra of R, giving the coefficients of the element with respect to the basis.
//     assert Order(P) eq R;
//     assert IsPrime(P);
//     // We produce the basis by lifting a basis of S/m(P)*S over R/P (here we are using Nakayama).
//     K,k:=ResidueField(P); // k:R->R/P=K
//     mPS:=Ideal(S,[m(g) : g in ZBasis(P)]);
//     zbmPS:=ZBasis(mPS);
//     d:=Ilog(#K,Integers()!Index(S,mPS));
//     V:=KModule(K,d); // V = S/m(P)*S vector space over K
// 
// 	zbS := ZBasis(S);
//     zbR := ZBasis(R);
//     mzbR := [ m(b) : b in ZBasis(R) ];
// 	N := #zbS;
// 	F := FreeAbelianGroup(N);
// 	relmPS := [F ! cc : cc in AbsoluteCoordinates(ZBasis(mPS),S)];
//     rel:=relmPS;
// 
//     A:=Algebra(S);
// 	mFS := map<F->A| x:->SumOfProducts(Eltseq(x),zbS)>;
// 	mSF := map<A->F| x:-> F ! AbsoluteCoordinates([x],S)[1]>;
// 	Q,q := quo<F|rel>; //q:F->Q. Q is an "abstract" abelian group isomorphic to S/P*S.
// 	bas := [];
// 	for i in [1..d] do
//     // for each iteration of the loop we mod-out from S the R-ideal generated by P*S and 
//     // the already found elements of the basis of S/P*S over R/P
// 		elt_F := (Q.1@@q);
// 		elt_S := mFS(elt_F);
// 		Append(~bas,elt_S);
//         rel_i:=[mSF(elt_S*bb) : bb in mzbR];
// 		rel := rel cat rel_i;
// 		Q, q := quo<F|rel>; //q:F->Q
// 	end for;
// 	assert IsTrivial(Q);
//     // mVS gives preimages S->S/P*S = V
// 	mVS := function(y)
//         y:=[m(Eltseq(y)[j]@@k) : j in [1..d]];
// 		return SumOfProducts(bas,y);
//     end function;
// 
//     // TODO CONTINUE FROM HERE, I think there is something missing here....
//     // function mSV using HNF: (\sum_i bas[i]*R)+P*S eq S. 
//     // Exploit this on ZBasis level to find the R-coordinates of ZBasis(S) wrt bas[i]'s 
//     gens:=&cat[[ b*z: z in mzbR ] : b in bas];
//     mat:=MatrixAtoQ(gens cat zbmPS);
//     den:=Denominator(mat);
//     dmat:=crQZ(den*mat);
//     H,Tr:=HermiteForm(dmat);//Tr*M eq H
//     HI,TrI:=HermiteForm(crQZ(den*MatrixAtoQ(zbS)));
//     TrI:=crZQ(TrI);
//     Tr1:=crZQ(Matrix(Rows(Tr)[1..#zbS]));
//     C:=TrI^-1*Tr1;
//     new_coords_zbS:=[];
//     for k in [1..#zbS] do
//         zbSk:=[];
...
//                 // TODO This sould be in the image of m(R), so I need to pullback by the image of R.
//                 // this is certainly doable...but annoying I guess.
//             Append(~zbSk,coord_i);
//         end for;
//         Append(~new_coords_zbS,zbSk);
//     end for;
//     mSV:=function(x)
//         xinS:=AbsoluteCoordinates([x],S)[1];
//         coords_inR:=[ SumOfProducts(xinS,[new_coords_zbI[i][k]: i in [1..#zbI]])  : k in [1..d]];
...
//-> SIGMA HERE: done
    ...
    for inu->nu in pl_01_E do
        ...
        // 20240702 the next line needs to be doublechecked
        sigma_U:=hom<U->U | [U.i@@U_pr@qI@sigma@@qI@U_pr : i in [1..Ngens(U)]]>; 
        // I am not sure if I should go through the direct sum decomposition QI = \sum_nu Q ???
        ...
        //assert pr(&*[ i eq 1 select alpha_A else sigma(Self(i-1)) : i in [1..a] ]) eq pi_Q; //sigma not on A
