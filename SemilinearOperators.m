...
{Given an isogeny class isog, a place nu of the DeligneAlgebra and a positive integer m, returns an integral element alpha of the DieudonneAlgebra A whose image in the quotient (OA/p^mOA)_nu is congruent to the nu-component alpha_nu of an element of of W-type, that is, such that alpha'_nu=(1,....,1,u) where N_(LE_nu/E_nu)(u)=pi_nu. Moreover, it returns also the product of maximal ideals of A above nu, raised to the power m*e where e is the common ramification index.}
    ...
    if not IsDefined(isog`AlphaWType,nu_Hash) then
        ...
        // The precision is chosen to be a majorative of RamificationIndex(pp)*m -- to match the quotient 
...
intrinsic AlphaDualAtNonConjStablePlace(isog::IsogenyClassFq,nu::AlgEtQIdl,m::RngIntElt)->AlgEtQElt,AlgEtQIdl
{Given an isogeny class isog, a non-conjugate stable place nu of the DeligneAlgebra and a positive integer m, returns // TODO 
// an integral element alpha of the DieudonneAlgebra A whose image in the quotient (OA/p^mOA)_nu is congruent to the nu-component alpha_nu of an element of of W-type, that is, such that alpha'_nu=(1,....,1,u) where N_(LE_nu/E_nu)(u)=pi_nu. Moreover, it returns also the product of maximal ideals of A above nu, raised to the power m*e where e is the common ramification index.
}
    ...
    if not IsDefined(isog`AlphaDualAtNonConjStablePlace,nu_Hash) then
        ...
        // 20260724 new stuff for q_alpha_nu
...
intrinsic AlphaDualAtConjStablePlaceRhoId(isog::IsogenyClassFq,nu::AlgEtQIdl,m::RngIntElt)->AlgEtQElt,AlgEtQIdl
...
// alpha_nu of W-type and delta_nu ...
    ...
    if not IsDefined(isog`AlphaDualAtConjStablePlaceRhoId,nu_Hash) then
        //TODO use the same code as AlphaWType together with delta_nu_tot_real_at_precision_conj_stable defined 
        // at line 431 SemilinearOperators_test_branch_Duals_gammas.m
...
intrinsic AlphaDualAtConjStablePlaceRhoNotId(isog::IsogenyClassFq,nu::AlgEtQIdl,m::RngIntElt)->AlgEtQElt,AlgEtQIdl
...
, and a precision m0, returns Qm0,qm0,FQm0,VQm0 where Qm0 is isomorphic to direct sum of (J/p^m0*J)_nu where nu runs over all places above p, qm0:J->Qm0 is the natural projection and FQm0,VQm0 are the reductions of F,V to Qm0. Moreover the intrinsic returns also the ideal den_ideal so that Qm0=J/den_ideal, and m0 and J.}
    if not assigned isog`SemilinearOperatorsDualComp then
        p:=CharacteristicFiniteField(isog);
        a:=Ilog(p,FiniteField(isog));
        _,_,_,_,A,pi_A,OA,_,WR,A_as_vector_space_over_L_data,OA_as_abelian_group_data:=DieudonneAlgebraCommEndAlg(isog); //TODO need all of them?
        ...
        //      - We take alpha_nu=(1,...,1,u_nu) of W-type, computed at precision m2.
        ...
        //      - So we need to compute delta_nu^-1 and hence also alpha_nu at precision m3:=(m0+1)+(g-1)=m0+g.
        //      - Since g divides a, to avoid dependecies from nu, we can just use m3=m2=m0+a.
        ...
        //      - TODO comments about precision?
        //      - We designed the intrinsic AlphaDualAtConjStablePlaceRhoNotId for this purpose.

        // TODO Following the approach starting from line 783 of SemilinearOperators_test_branch_Duals_gammas.m to 
        //      compute FQm0_1 and FQm0.
        delta_inv:=AssociativeArray();
        images_gens_Qm0:=AssociativeArray();
        images_gens_Qm0_1:=AssociativeArray();
        PPms:=AssociativeArray();
        m2:=m0+a;
        //FIXME check m0 vs m1 vs m2
        for pair in conj_pairs do
            ...
            e:=RamificationIndex(PPs[1]);
            PPm:=(&*PP)^(m2+e);
            delta_inv[myHash(nu)]:=One(A);
            delta_inv[myHash(nub)]:=One(A);
            PPms[myHash(nu)]:=PPm;
            PPms[myHash(nub)]:=bar_onA(PPm);
            images_gens_Qm0[myHash(nu)]:= //TODO
            images_gens_Qm0_1[myHash(nu)]:= //TODO
            images_gens_Qm0[myHash(nub)]:= //TODO this needs the mult by p^(a-1), see commented stuff below
            images_gens_Qm0_1[myHash(nub)]:= //TODO
        end for;
        for nu in rho_id do
            alpha_nu,delta_inv_nu:=AlphaDualAtConjStablePlaceRhoId(isog,nu,m2);
            PPs:=PlacesOfDieudonneAlgebraAbovePlaceOfQF(isog,nu);
            e:=RamificationIndex(PPs[1]);
            PPm:=(&*PP)^(m2+e);
            delta_inv[myHash(nu)]:=delta_inv_nu;
            PPms[myHash(nu)]:=PPm;
            images_gens_Qm0[myHash(nu)]:= //TODO
            images_gens_Qm0_1[myHash(nu)]:= //TODO
        end for;
        for nu in rho_notid do
            alpha_nu:=AlphaDualAtConjStablePlaceRhoNotId(isog,nu,m2);
            PPs:=PlacesOfDieudonneAlgebraAbovePlaceOfQF(isog,nu);
            e:=RamificationIndex(PPs[1]);
            PPm:=(&*PP)^(m2+e);
            delta_inv[myHash(nu)]:=One(A);
        ...
        J_Jnus,Jnus_J:=ChineseRemainderTheoremFunctions(JOA,[PPms[nu]:nu in Keys(PPms)]); //here we use m2 instead of m0 FIXME ?
...
//            return out;
//        end function;
        // FIXME image_qq should be replaced
        ...
        //TODO add all assert and assert2
